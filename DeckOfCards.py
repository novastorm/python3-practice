...
# building the common API that the game developers will use.
...
#  - Deal a card from a deck: return the first card of the deck and remove that card
#    from the deck so that it can't be dealt again
...
class Deck:
  def __init__(self, cardClass, numberOfDecks = 1):
    ...
    self.numberOfCards = self.numberOfDecks * len(self.cardClass.Suit) * len(self.cardClass.Face)
